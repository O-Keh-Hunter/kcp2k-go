...
// checkAndRequestMissingFrames 检查并请求缺失的帧
func (c *LockStepClient) checkAndRequestMissingFrames() {
	// 检查是否需要补帧
	nextFrameID := c.lastFrameID + 1

	if c.currentFrameID > c.lastFrameID {
		// 有缺失的帧，请求补帧
		err := c.requestFramesInternal(nextFrameID, c.currentFrameID-1)
		if err != nil {
			c.logger.Printf("Failed to request missing frames: %v", err)
		}
	}
}
...
func (c *LockStepClient) handleGameStart(payload []byte) {
	...
	if len(payload) > 0 {
		...
		if err != nil {
			...
			// 如果游戏已经在运行，设置合理的补帧起始点
			if gameStartMsg.GameAlreadyRunning && gameStartMsg.CurrentFrameId > 0 {
				c.mutex.Lock()

				c.currentFrameID = FrameID(gameStartMsg.CurrentFrameId)
				// 设置合理的补帧起始点，避免请求过多历史帧
				// 通常只需要补最近的几帧即可
				catchupFrames := FrameID(10) // 最多补10帧
				if c.currentFrameID > catchupFrames {
					c.lastFrameID = c.currentFrameID - catchupFrames
				} else {
					c.lastFrameID = 0
				}
				c.mutex.Unlock()
...
func (c *LockStepClient) PopFrame() *Frame {
	...
	if !exists {
		// 当需要的帧不存在时，检查并请求缺失的帧
		c.checkAndRequestMissingFrames()
		return nil
	}
